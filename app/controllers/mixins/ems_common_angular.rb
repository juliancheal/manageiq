module Mixins
  module EmsCommonAngular
    ...
    def set_ems_record_vars(ems, mode = nil)
      ...
      build_connection(ems, endpoints)
    end

    def build_connection(ems, endpoints)
      authentications = build_credentials(ems)
      configurations = []

      [:default, :amqp, :ssh_keypair, :metrics, :hawkular].each do |role|
        configurations << build_configuration(authentications, endpoints, role) if authentications[role]
      end
